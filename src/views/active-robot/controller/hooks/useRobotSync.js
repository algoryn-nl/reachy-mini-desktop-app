  ...
  useEffect(() => {
    ...
    if (data.head_pose) {
      ...
      const canSyncFromRobot =
        !isDraggingRef.current && !isUsingGamepadKeyboardRef.current && !hasRecentUserInteraction; // Never sync if user recently interacted
      ...
      if (canSyncFromRobot) {
        ...
        const xChanged =
          Math.abs(newState.headPose.x - currentLocalValues.headPose.x) > majorChangeTolerance;
        const yChanged =
          Math.abs(newState.headPose.y - currentLocalValues.headPose.y) > majorChangeTolerance;
        const zChanged =
          Math.abs(newState.headPose.z - currentLocalValues.headPose.z) > majorChangeTolerance;
        const pitchChanged =
          Math.abs(newState.headPose.pitch - currentLocalValues.headPose.pitch) >
          majorChangeTolerance;
        const yawChanged =
          Math.abs(newState.headPose.yaw - currentLocalValues.headPose.yaw) > majorChangeTolerance;
        const rollChanged =
          Math.abs(newState.headPose.roll - currentLocalValues.headPose.roll) >
          majorChangeTolerance;
        const bodyYawChanged =
          Math.abs(newState.bodyYaw - currentLocalValues.bodyYaw) > majorChangeTolerance;
        const antennaLeftChanged =
          !currentLocalValues.antennas ||
          Math.abs(newState.antennas[0] - (currentLocalValues.antennas[0] || 0)) >
            majorChangeTolerance;
        const antennaRightChanged =
          !currentLocalValues.antennas ||
          Math.abs(newState.antennas[1] - (currentLocalValues.antennas[1] || 0)) >
            majorChangeTolerance;
    ...
    antennasRef,
    targetSmoothingRef,
  ]); // localValues removed from deps to prevent infinite loop - we use ref instead
}
