...
echo -e "${BLUE}🚀 Building update manifest for ${ENV} environment${NC}"
...
if [[ "$PLATFORM" == darwin-* ]]; then
    ...
    if [ -d "$MACOS_DIR" ]; then
        ...
        if [ -n "$UPDATER_FILE" ] && [ -f "$UPDATER_FILE" ]; then
            ...
            if [ -f "$SIG_FILE" ]; then
                ...
                echo -e "${GREEN}✅ Found macOS updater artifact: ${FILE_NAME}${NC}"
            fi
        fi
    fi
    ...
    if [ -z "$UPDATER_FILE" ] || [ ! -f "$UPDATER_FILE" ]; then
        ...
        if [ -d "$NSIS_DIR" ]; then
            ...
            if [ -n "$UPDATER_FILE" ] && [ -f "$UPDATER_FILE" ]; then
                ...
                if [ -f "$SIG_FILE" ]; then
                    FILE_NAME=$(basename "$UPDATER_FILE")
                    echo -e "${GREEN}✅ Found Windows NSIS updater artifact: ${FILE_NAME}${NC}"
                fi
            fi
        fi
    fi
    ...
    if [ -d "$APPIMAGE_DIR" ]; then
        ...
        if [ -n "$UPDATER_FILE" ] && [ -f "$UPDATER_FILE" ]; then
            ...
            if [ -f "$SIG_FILE" ]; then
                FILE_NAME=$(basename "$UPDATER_FILE")
                echo -e "${GREEN}✅ Found Linux updater artifact: ${FILE_NAME}${NC}"
            fi
        fi
    fi
fi
...
if [ -z "$UPDATER_FILE" ] || [ ! -f "$UPDATER_FILE" ]; then
    echo -e "${RED}❌ Updater artifact not found!${NC}"
...
echo -e "${BLUE}📋 Copying updater artifact to ${OUTPUT_DIR}/${NC}"
...
# Generate JSON
echo ""
echo -e "${BLUE}📄 Generating update metadata...${NC}"
